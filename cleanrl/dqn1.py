...
def flatten_obs(obs_dict):
    image = obs_dict["image"].reshape(obs_dict["image"].shape[0], -1)
    return image
...
if __name__ == "__main__":
    ...
    writer = SummaryWriter(f"runs/minigriddqn/{run_name}")
    ...
    flat_obs_space = Box(low=0, high=255, shape=(7 * 7 * 3,), dtype=np.uint8)
    rb = ReplayBuffer(
        args.buffer_size,
        flat_obs_space,
    ...
    for global_step in range(args.total_timesteps):
        ...
        if random.random() < epsilon:
            ...
            obs_tensor = torch.tensor(flatten_obs(obs)).to(device)
        ...
        if "final_info" in infos:
            for info in infos["final_info"]:
                if info and "episode" in info:
                    ...
                    writer.add_scalar("charts/episodic_return_1", total_return, global_step)
                    print(f"global_step={global_step}, episodic_return={total_return}, episodic_return = {ret}")
                    total_return=0
        ...
        rb.add(flatten_obs(obs), flatten_obs(real_next_obs), actions, rewards, terminations, infos)
        obs = next_obs
