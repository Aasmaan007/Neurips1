...
class Args:
   ...
   model_path = "runs/checkpoints/qmaml/Hopper-v4__MAML_Q__1__2025-08-26_16-46-58__1756207018/latest.pth"
...
class QNetwork(nn.Module):
    def __init__(self, env):
        ...
        self.embedding = nn.Sequential(
            ...
            nn.Linear(256, 256),
            nn.ReLU(),
            nn.Linear(256, 1),  # 16-dim embedding
        )
...
if __name__ == "__main__":
   ...
   run_name = f"{args.env_id}_{args.exp_name}_{args.seed}_{args.lambda_kl_init}_{args.lambda_kl_final_frac}_maxconfigwithentropy{int(time.time())}"
   if args.track:
       import wandb
   ...
   if(args.pretrained):
        ...
        sf_state_dict = checkpoint2["qmeta_network_state_dict"]
        mapped_state_dict = {}
        mapped_state_dict["embedding.0.weight"] = sf_state_dict["fc1.weight"]
        mapped_state_dict["embedding.0.bias"]   = sf_state_dict["fc1.bias"]
        mapped_state_dict["embedding.2.weight"] = sf_state_dict["fc2.weight"]
        mapped_state_dict["embedding.2.bias"]   = sf_state_dict["fc2.bias"]
        mapped_state_dict["embedding.4.weight"] = sf_state_dict["fc3.weight"]
        mapped_state_dict["embedding.4.bias"]   = sf_state_dict["fc3.bias"]
   ...
   for global_step in range(args.total_timesteps):
       ...
       if global_step > args.learning_starts:
           ...
           # ------- start replacement block -------
                   ...
                   with torch.no_grad():
                       qmeta_rep = qmeta_net(obs_rep, pi_rep).view(-1)      # [B*K]; qmeta_net is your meta Qnet (must exist)
                   ...
                   with torch.no_grad():
                    ...
                    #    ema_qmin_std  = (1.0 - ema_alpha) * ema_qmin_std  + ema_alpha * (batch_qmin_std + eps_small)
                        ...
                        # per-state means/stds (population std for stability)
                        per_mu_meta = qmeta_rs.mean(dim=1)                   # [B]
                        per_sd_meta = qmeta_rs.std(dim=1, unbiased=False)    # [B]
                        per_mu_min  = qmin_rs.mean(dim=1)                    # [B]
                        per_sd_min  = qmin_rs.std(dim=1, unbiased=False)     # [B]

                        # robust aggregation across states
                        ...
                        # EMA updates
                        ema_mu_meta = (1-ema_alpha)*ema_mu_meta + ema_alpha*batch_mu_meta if 'ema_mu_meta' in locals() else batch_mu_meta
                        ema_sd_meta = (1-ema_alpha)*ema_sd_meta + ema_alpha*(batch_sd_meta + eps_small) if 'ema_sd_meta' in locals() else (batch_sd_meta + eps_small)
                        ema_mu_min  = (1-ema_alpha)*ema_mu_min  + ema_alpha*batch_mu_min  if 'ema_mu_min'  in locals() else batch_mu_min
                        ema_sd_min  = (1-ema_alpha)*ema_sd_min  + ema_alpha*(batch_sd_min + eps_small) if 'ema_sd_min'  in locals() else (batch_sd_min + eps_small)
                ...
                #    qmeta_scaled_rep = qmeta_rep * scale   # [B*K]
                   scale = ema_sd_min / (ema_sd_meta + eps_small)
                   shift = ema_mu_min - scale * ema_mu_meta
                   qmeta_scaled_rep = qmeta_rep * 1 #scale + shift
                   ...
                   with torch.no_grad():
                    ...
                    #    keep_mask_rs = (qmeta_rs_scaled >= p_lo) & (qmeta_rs_scaled <= p_hi)  # [B,K]
                         ...
                         p_lo = torch.quantile(qmeta_scaled_flat, p_low)      # scalars
                         p_hi = torch.quantile(qmeta_scaled_flat, p_high)
                         keep_mask_rs = (qmeta_scaled_flat >= p_lo) & (qmeta_scaled_flat <= p_hi)  # [B*K] boolean
                   ...
                   qmeta_scaled_flat = qmeta_scaled_rep.view(-1)  # [B*K]
                   qmeta_scaled_flat_masked = qmeta_scaled_flat.clone()
                   qmeta_scaled_flat_masked[~keep_mask] = 0.0      #
                   ...
                   sac_actor_loss = ((alpha * log_pi_flat) - torch.max(min_q_pi_rep,qmeta_scaled_flat_masked)).mean()
           ...
           if global_step % 100 == 0:
               ...
               writer.add_scalar("meta/qmeta_raw", qmeta_rep.mean().item(), global_step)
               ...
               writer.add_scalar("meta/qmeta_scaled_masked_all0", qmeta_scaled_flat_masked.mean().item(), global_step)
