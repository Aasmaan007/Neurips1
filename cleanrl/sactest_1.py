...
class Args:
   ...
   model_path = "runs/checkpoints/maml/Hopper-v4__MAML_SF__1__2025-08-27_18-55-06__1756301106/latest.pth"
...
class SFSoftQNetwork(nn.Module):
    def __init__(self, env):
        super().__init__()
        state_dim = np.prod(env.single_observation_space.shape)
        action_dim = np.prod(env.single_action_space.shape)
...
if __name__ == "__main__":
   ...
   run_name = f"{args.env_id}_{args.exp_name}_{args.seed}_{args.lambda_kl_init}_{args.lambda_kl_final_frac}_initfromqonlineconfigwithentropy{int(time.time())}"
   if args.track:
       import wandb
   ...
   qf1 = Qonlinenet(envs, 6).to(device)
...
#    qf1 = qf1.to(device)
   ...
   qf2 = Qonlinenet(envs,6).to(device)
   qf2.load_state_dict(qf1.state_dict())
   qf1_target = Qonlinenet(envs,6).to(device)
   qf2_target = Qonlinenet(envs,6).to(device)
   ...
   for global_step in range(args.total_timesteps):
       ...
       if global_step > args.learning_starts:
           ...
           with torch.no_grad():
               ...
               qf1_next_target = qf1_target(data.next_observations, next_state_actions, extra_vec)
               #qf1_next_target = torch.einsum("bd,d->b", qf1_next_targetpsi, w) 
               qf2_next_target = qf2_target(data.next_observations, next_state_actions, extra_vec)
           ...
           qf1_a_values = qf1(data.observations, data.actions, extra_vec).view(-1)
           qf2_a_values = qf2(data.observations, data.actions, extra_vec).view(-1)
           ...
           # ------- start replacement block -------
                   ...
                   qf1_pi_rep = qf1(obs_rep, pi_rep, extra_vec.repeat_interleave(K, dim=0)).view(-1)               # [B*K]
                   qf2_pi_rep = qf2(obs_rep, pi_rep, extra_vec.repeat_interleave(K, dim=0)).view(-1)               # [B*K]
