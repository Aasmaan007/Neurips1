...
from cleanrl.diayn.models_cont import FeatureNetwork, SFNetwork, QNetwork, Actor, Critic
...
class Args:
    ...
    env_data_path: str = "runs/data/Walker2d-v4__ddpg_continuous_action__1__1755422051/task_regression_data.pkl"
    ...
    workers: int = 4
    dropout: float = 0.15
    model_path2: str = "runs/checkpoints/maml/Walker2d-v4__MAML_SF__1__2025-08-19_22-18-58__1755622138/latest.pth" 
...
def train():
    ...
    qnet = Critic(dummy_env, args.n_skills_selected).to(device)
    actor = Actor(dummy_env, args.n_skills_selected).to(device)

    checkpoint2 = torch.load(args.model_path2)
    sf_network = SFNetwork(dummy_env.observation_space.shape[0], dummy_env.action_space.shape[0], args.sf_dim).to(device)
        ...
        for batch_idx, (s, a, r, snext, terminated) in enumerate(task_loader):
            ...
            if epoch % args.task_lag == 0:
                with torch.no_grad():
                    ...
                    # print("snext shape:", snext.shape)
                    # print("action shape:", action.shape)
                    sf_sa = sf_network.sf_vector(s, a)
                    action = torch.tensor(action, dtype=torch.float32, device=snext.device)
                    sf_snext_anext = sf_network.sf_vector(snext, action)
                    gamma = 0.99
