...
"""
    ...
    q_network = Critic(env , args.n_skills).to(device)
    optimizer = optim.Adam(q_network.parameters(), lr=args.learning_rate_qnet)
    target_network = Critic(env , args.n_skills).to(device)
    target_network.load_state_dict(q_network.state_dict())


    actor = Actor(env , args.n_skills).to(device)
        ...
        for steps in range(args.max_timesteps+5):
            ...
            if global_step > args.learning_starts:
                ...
                if(global_step % args.train_frequency == 0):
                    if args.env_id!="Swimmer-v4" and args.env_id!="Reacher-v4":
                    ...
                    # Update Q_network
                    loss, old_val , intrinsic_rewards , logqz , td_target , bootstrapping = train_dqn(q_network, actor, target_network, target_actor, discriminator , data, device, args, global_step , optimizer, actor_opt)
