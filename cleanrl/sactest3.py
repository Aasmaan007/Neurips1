...
def get_all_pairs(states):
    ...
    for s,a in states:
        all_states.append(s)
    return torch.tensor(np.stack(all_states), dtype=torch.float32)
...
def meta_kl_loss(actor, qf1, qf2, qmeta_net, w, obs_batch, K, p_lo, p_hi, ema_state, ema_alpha=0.02):
    """
    ...
    with torch.no_grad():
        ...
        qmeta_scaled = qmeta_rep * scale + shift
...
if __name__ == "__main__":
    ...
    states = get_all_pairs(state_data)
    ...
    if args.pretrain:
        ...
        for epoch in range(1, num_epochs + 1):
            ...
            for (batch_states_cpu,) in train_loader:
                ...
                kl_term, frac_kept, scale_used = meta_kl_loss(
                    actor=actor, qf1=qf1, qf2=qf2, qmeta_net=qmeta_net, w=w,
        ...
        obs, _ = envs.reset(seed=args.seed)
        print("==> Pretrain done. Proceeding to PURE SAC...")
