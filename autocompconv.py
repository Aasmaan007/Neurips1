...
def extract_sf_seed_pretrained(filename):
    match = re.search(r"v4__ddpgtest__([0-9]+)__(True|False)", filename)
    if not match:
        raise ValueError(f"Invalid filename format: {filename}")
    return int(match.group(1)), match.group(2) == "True"

def extract_q_seed_pretrained(filename):
    match = re.search(r"__(\d+)__(True|False)", filename)
    return int(match.group(1)), match.group(2) == "True"
...
def main(args: Args):
    ...
    ALL_THRESHOLDS = range(8000, 10000)
    ...
    for patience in ALL_PATIENCES:
        for threshold in ALL_THRESHOLDS:
            ...
            if(positive_count > 4 and np.mean(diff10)> best_result):
